...
def auswertung():
    ...
    con = sqlite3.connect("gui_highscore.db")
    cursor = con.cursor()
    sql = "INSERT INTO daten VALUES('" + lbname["text"] +"'," + str(differenz) + ")"
    cursor.execute(sql)
    con.commit()
    con.close()


    # Highscore sortiert laden
    con = sqlite3.connect("gui_highscore.db")
    cursor = con.cursor()
    ...
    i = 1
    for dsatz in cursor:
        ausgabe += str(i) + ". " + dsatz[0] + " " + str(round(dsatz[1],2)) + " sec.\n"
        i = i + 1
...
for i in range(5):
    ...
    # Aufgabenstellung
    tkinter.Label(main, text=str(i+1)+".").grid(row=i+2, column=0)
